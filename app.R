...
server <- function(input, output, session) {
  ...
  observeEvent(input$raw_file, {
    ...
    if (is_ix) {
      ...
      tryCatch({
        ...
        if (length(matched) == 0) matched <- character(0)
        updateCheckboxGroupInput(session, "bama_type_select", selected = matched)
  ...
  output$plate_map_heatmap <- renderPlot({
    ...
    ggplot(grid, aes(x = Col, y = Row, fill = TypeColor)) +
      ...
      geom_text(aes(label = Label, color = label_col), size = 2.2,
  ...
  output$pb_antigen_selector_ui <- renderUI({
    ...
    selectInput("pb_selected_antigen", NULL,
                choices = choices, selected = ag_cols[1], width = "100%")
  })
  ...
  tit_base_data <- reactive({
    ...
    df_full  <- mfi_list$full;  req(!is.null(df_full))
    helper   <- rv$helper_edited
    is_ix    <- !is.null(rv$instrument) && rv$instrument == "INTELLIFLEX"
  ...
  # Reactive: filter tit_base_data to standard_curve rows only ----------------
  ...
  sc_plot_reactive <- reactive({
    ...
    pal        <- setNames(.make_pal(length(samp_grps)), samp_grps)
    ...
    # -- Concentration mode: numeric x-axis with dilution secondary axis ------
      ...
      p <- ggplot(d, aes(x      = x_value,
                         ...
                         fill   = base_sample_id,
                         group  = base_sample_id)) +
        geom_line(linewidth = 0.55, linetype = "dashed", alpha = 0.55) +
    ...
    # -- LOD line from Point-based QC configuration ---------------------------
    lod_val <- tryCatch(pb_cfg()$lod, error = function(e) NULL)
    if (!is.null(lod_val) && !is.na(lod_val) && lod_val > 0) {
      p <- p +
        geom_hline(yintercept = lod_val, linetype = "dashed",
                   colour = "#e74c3c", linewidth = 0.75, alpha = 0.85) +
        annotate("text",
                 x     = if (use_dil_labels) Inf else -Inf,
                 y     = lod_val,
                 label = paste0("LOD = ", formatC(lod_val, format = "fg", digits = 4)),
                 hjust = if (use_dil_labels) 1.05 else -0.08,
                 vjust = -0.45,
                 size  = 3.1, colour = "#e74c3c", fontface = "italic",
                 inherit.aes = FALSE)
    }
  ...
  sc_sample_conc_all <- reactive({
    req(rv$analysis_run)
    d_all  <- tit_base_data(); req(!is.null(d_all))
